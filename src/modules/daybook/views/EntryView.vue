<template>
  <template v-if="entry">
    <div class="entry-title d-flex justify-content-between p-2">
      <div>
        <span class="text-success fa-3 fs-3 fw-bold">
          {{ dateFormatEntry.day }}
        </span>
...
import MediaUploadService from "../services/uploadImage-service";
  ...
  methods: {
    ...
    async saveEntry() {
      ...
      if (this.fileSelected) {
        let formBodyCloudinary = new FormData();
        formBodyCloudinary.append("upload_preset", "vue-journal");
        formBodyCloudinary.append("file", this.fileSelected);
        const data = await MediaUploadService.uploadImageToCloudinary(
          formBodyCloudinary
        );
        this.entry.picture = data.data.secure_url;
